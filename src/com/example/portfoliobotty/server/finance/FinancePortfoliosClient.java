...
package com.example.portfoliobotty.server.finance;
...
public class FinancePortfoliosClient {
	private static final Logger LOG = Logger.getLogger(FinancePortfoliosClient.class.getName());
  ...
  public static Portfolio initPortfolioEntry(PortfolioEntry portfolioEntry) {
	  ...
	  if (portfolioEntry.getFeedLink().getFeed() == null) {
		  ...
		  for (int i = 0; i < inlinedFeed.getEntries().size(); i++) {
			  PositionEntry positionEntry = inlinedFeed.getEntries().get(i);
			  Position position = initPositionEntry(positionEntry);
	  ...
	  String currencyCode = portfolioData.getCurrencyCode();
	  double gainPercentage = portfolioData.getGainPercentage();
	  //  Returns:
	  double return1w = portfolioData.getReturn1w();
	  double return4w = portfolioData.getReturn4w();
	  double return3m = portfolioData.getReturn3m();
	  double returnYTD = portfolioData.getReturnYTD();
	  double return1y = portfolioData.getReturn1y();
	  double return3y = portfolioData.getReturn3y();
	  double return5y = portfolioData.getReturn5y();
	  double returnOverall = portfolioData.getReturnOverall();
	  Returns returns = new Returns(return1w, return4w, return3m, returnYTD, return1y, return3y, return5y, returnOverall);
	  ...
	  Portfolio portfolio = new Portfolio(portfolioEntry.getId(), title,currencyCode, gainPercentage, costBasis, daysGain, gain, marketValue, returns, positions);
	  return portfolio;
  }

  
  /**
   * Inits Position from a feed
   *
   * @param positionEntry The position entry of interest
   */
  private static Position initPositionEntry(PositionEntry positionEntry) {
	String exchange = positionEntry.getSymbol().getExchange();
	String symbol = positionEntry.getSymbol().getSymbol();
	String fullName = positionEntry.getSymbol().getFullName();
	
    PositionData positionData = positionEntry.getPositionData();
    double shares = positionData.getShares();
    double gainPercentage = positionData.getGainPercentage();
    
//  Returns:
    double return1w = positionData.getReturn1w();
    double return4w = positionData.getReturn4w();
    double return3m = positionData.getReturn3m();
    double returnYTD = positionData.getReturnYTD();
    double return1y = positionData.getReturn1y();
    double return3y = positionData.getReturn3y();
    double return5y = positionData.getReturn5y();
    double returnOverall = positionData.getReturnOverall();
    Returns returns = new Returns(return1w, return4w, return3m, returnYTD, return1y, return3y, return5y, returnOverall);
    ...
    Position position = new Position(positionEntry.getId(), exchange, symbol, fullName, shares, gainPercentage, costBasis,  daysGain, gain, marketValue, returns);
	return position;
  }
   ...
   * @throws ServiceException If the service is unable to handle the request.
   */
  public static void queryPortfolioFeed(FinanceService service, String feedUrl)
   ...
   * @throws ServiceException If the service is unable to handle the request.
   */
  public static void queryPositionFeed(FinanceService service, String feedUrl)
      throws IOException, MalformedURLException, ServiceException {
    System.out.println("Requesting Feed at location " + feedUrl);
   ...
   * @return login success or failure
   */
  public static FinanceService loginUser(String token,PrivateKey privateKey) {
	  FinanceService service = new FinanceService("financebotty");
    if(privateKey != null){
  ...
  public static void run(String token, PrivateKey privateKey) {
	    // Google Finance GData service.
	    // Set username and password from command-line arguments if they were passed
	    // in. Otherwise prompt the user to login.
	    FinanceService service = loginUser(token, privateKey);
	    server = LoginServlet.FINANCE_SCOPE_URL;
	    baseUrl = server + basePath + "default";
	    printMainMenuHelp();
	    Scanner sc = new Scanner(System.in);
	    System.out.println("Sample Google Finance Portfolios Client");
